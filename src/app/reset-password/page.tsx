...
function ResetPasswordInner() {
  ...
  // ⬅️ Read BOTH uid and token from the URL the email provided
  const uid = params.get("uid");     // e.g. ?uid=Mg
  const token = params.get("token"); // e.g. ?token=abc-...
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: { uid: string; token: string; new_password: string }) => {
      // NOTE: if your axios baseURL is "/api", use "/users/reset-password/"
      // otherwise use "/api/users/reset-password/"
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!uid || !token) {
      setError("Missing reset link parameters. Please use the link from your email.");
      return;
    }
    ...
    // ⬅️ Send the exact fields the DRF view requires
    mutate({ uid, token, new_password: form.password });
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {(!uid || !token) && (
              <p className="rounded-xl border border-amber-100 bg-amber-50 px-3 py-2 text-sm text-amber-700">
                Missing uid/token. Please use the reset link from your email.
              </p>
            )}
